	...
	ext().execute_with(|| {
        with_all_maps!(
            for map with name as name: {
                ...
                assert_decoded_eq!(
                    MetaStorageReader::<Runtime>::execute(&mut MockHandle::new(input.encode(), Some(30_000), DUMMY_CTX)),
...
fn costs() {
	ext().execute_with(|| {
		...
		let mut handle = MockHandle::new(input.encode(), Some(30_000), DUMMY_CTX);
